...
function downloadGameset(gsName, status) {
	if (action_available) {
		...
		if (confirm(message)) {
			...
			fileSystem.root.getDirectory(directoryName+'/'+gamesetDir+'/'+gsName, {create: true, exclusive: false}, function() {
				gs = getGameSet(gsName, gamesetHosted.gameset);
				if (gs != null) {
					dwprogress = 0;
			...
			function() {
				alert(texte_gameset_create_error[game_options.lang]+$.ucfirst(gsName)+" !");
				action_available = true;
			});
		}
	}
...
function deleteGameset(gsName) {
	if (action_available) {
		...
		if (confirm(message)) {
			...
			$('.btn_gg').addClass("ui-disabled");
			fileSystem.root.getDirectory(directoryName+'/'+gamesetDir+'/'+gsName, {create: true, exclusive: false}, function(direntry) {
				direntry.removeRecursively(function() {
					...
					alert(texte_gameset_delete_error[game_options.lang]+$.ucfirst(gsName)+" !");
				});
			}, 
			function() {
				alert(texte_gameset_delete_error[game_options.lang]+$.ucfirst(gsName)+" !");
